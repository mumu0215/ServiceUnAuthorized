module src

go 1.15

require (
	github.com/PuerkitoBio/goquery v1.6.1
	github.com/bradfitz/gomemcache v0.0.0-20190913173617-a41fca850d0b
	github.com/chrispinkyang/golimitDemo v0.0.0-20200715133125-0223445cdfb1 // indirect
	github.com/dutchcoders/goftp v0.0.0-20170301105846-ed59a591ce14
	github.com/go-redis/redis v6.15.9+incompatible
	github.com/jinzhu/gorm v1.9.16
	github.com/json-iterator/go v1.1.10
	github.com/mitchellh/go-vnc v0.0.0-20150629162542-723ed9867aed // indirect
	github.com/pangudashu/memcache v0.0.0-20190519054813-4e579607dcd6 // indirect
	github.com/panjf2000/ants/v2 v2.4.4 // indirect
	github.com/samuel/go-zookeeper v0.0.0-20201211165307-7117e9ea2414 // indirect
	github.com/urfave/cli/v2 v2.3.0
	go.mongodb.org/mongo-driver v1.4.6
	k8s.io/apimachinery v0.21.0
	k8s.io/client-go v0.21.0 // indirect
)
